...
export const securityApi = {
  getStatus: () =>
    client.get<{ success: boolean; data: SecurityStatus }>("/security/status").then((r) => r.data),
  ...
  revokeSession: (sessionId: string) =>
    client.post(`/security/sessions/${sessionId}/revoke`).then((r) => r.data),
  ...
  initiateTwoFactorSetup: (password: string) =>
    client
      .post<{
        success: boolean;
        data: { uri: string; displayKey: string; secret: string; qrCodeDataUrl: string };
