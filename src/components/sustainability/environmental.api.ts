...
// Phase 4 — Historical Intelligence: extended with the raw sustainability
// inputs (renewableShare, greenCover, carbon) the backend now also
// aggregates per day, plus the day's EcoScore computed with the same
// engine as the live score (see ecoScore.service.ts on the backend).
...
// ─── Phase 4: 24-hour trend point (returned by /cities/:cityId/trend) ────────
// Raw (non-aggregated) reading — used for the "24 Hours" range of the
// Sustainability history chart, since daily aggregation is too coarse
...
export const environmentalApi = {
  ...
  // ── Phase 4: typed alias of getCityTrend, same endpoint, for the
  //    Sustainability history chart's "24 Hours" range. The untyped
  //    getCityTrend above is left exactly as-is for its existing caller
  //    (Dashboard's 24h AQI chart).
  getCityTrendTyped: (
    cityId: string,
    hours = 24,
  ): Promise<{
    success: boolean;
    data: { trend: CityTrendPoint[]; hours: number };
  }> => client.get(`/environmental/cities/${cityId}/trend`, { params: { hours } }).then((r) => r.data),
