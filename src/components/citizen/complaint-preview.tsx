/**
 * complaint-preview.tsx — Phase 12
 *
    ...
    >
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <h2 className="text-base font-semibold tracking-tight mt-0.5">Complaint Preview</h2>
        </div>
        <Button type="button" variant="outline" size="sm" onClick={onEdit} className="gap-1.5 shrink-0">
      ...
      <div className="grid grid-cols-2 gap-3">
        <PreviewSection icon={<Tag className="size-3.5" />} label="Category">
          <p className="text-sm font-medium">{humanizeIssueType(data.issueType)}</p>
        </PreviewSection>
        ...
        {data.files.length > 0 ? (
          <div className="flex flex-wrap gap-2">
            {data.files.map((f, i) => (
              <div
                key={i}
                className="flex items-center gap-1.5 text-xs rounded-lg border border-border/60 px-2.5 py-1.5 bg-muted/20"
              >
