...
function TabBar({
  ...
  return (
    <div className="flex items-center gap-0.5 p-1 bg-muted/50 rounded-2xl border border-border w-fit">
...
export function CitizenHubPage() {
  ...
  // When a complaint is opened from submit-success, switch to history tab first
  ...
  // Non-authenticated users see a welcome screen
  if (!isAuthenticated) {
    return (
          ...
          {[
            {
              ...
              desc: "Report environmental issues in your city with evidence and location.",
              icon: "📝",
            },
            {
  ...
  // Wrong role guard — show friendly message if authority/admin lands here
  if (user && user.role !== "citizen") {
    return (
      <div className="px-4 md:px-8 py-8 max-w-[1400px] mx-auto">
        <div className="glass rounded-2xl p-8 text-center space-y-3">
          <h2 className="text-lg font-semibold">
            This page is for citizens.
          </h2>
  ...
  return (
    <div className="px-4 md:px-8 py-6 space-y-5 max-w-[1400px] mx-auto">
      {/* ── Top bar ── */}
      ...
      {tab === "new" && (
        <Panel
          ...
          action={
            <Button variant="ghost" size="sm" className="text-xs" onClick={() => setTab("history")}>
              View My Complaints
            </Button>
          }
        >
