  ...
  return (
    <div className="px-4 md:px-6 py-6 space-y-5">
      <SectionTitle
        ...
        action={
          <Button variant="outline" size="sm" onClick={() => qc.invalidateQueries({ queryKey: ["pa-users"] })}>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <Input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Search name, email, phone…" className="pl-8 h-9" />
        ...
        <Button variant={showFilters ? "default" : "outline"} size="sm" className="h-9 gap-1.5" onClick={() => setShowFilters(v => !v)}>
          ...
          <div className="space-y-1.5">
            {users.map(u => (
              <div key={u._id} className="flex items-center gap-4 p-3.5 rounded-xl border border-border/60 bg-card hover:bg-muted/30 transition-colors">
                ...
                <div className="text-xs text-muted-foreground shrink-0 hidden md:block w-20 text-right">{format(new Date(u.createdAt), "MMM d, yyyy")}</div>
                <div className="flex items-center gap-1 shrink-0">
                  {u.isActive
                    ? <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-muted-foreground hover:text-destructive" title="Deactivate" onClick={() => setPending({ user: u, action: "deactivate" })}><PowerOff className="size-3.5" /></Button>
                    : <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-muted-foreground hover:text-success" title="Activate" onClick={() => setPending({ user: u, action: "activate" })}><Power className="size-3.5" /></Button>}
                  <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-muted-foreground hover:text-warning" title="Lock" onClick={() => setPending({ user: u, action: "lock" })}><Lock className="size-3.5" /></Button>
                  <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-muted-foreground hover:text-info" title="Unlock" onClick={() => setPending({ user: u, action: "unlock" })}><Unlock className="size-3.5" /></Button>
                </div>
              </div>
            ))}
          </div>
        ...
        {data && data.pagination.pages > 1 && (
          <div className="flex items-center justify-between pt-4">
            ...
            <div className="flex gap-2">
              <Button variant="outline" size="sm" disabled={page <= 1} onClick={() => setPage(p => p - 1)}><ChevronLeft className="size-3.5 mr-1" />Prev</Button>
              <Button variant="outline" size="sm" disabled={page >= data.pagination.pages} onClick={() => setPage(p => p + 1)}>Next<ChevronRight className="size-3.5 ml-1" /></Button>
            </div>
          </div>
        )}
      </div>
      ...
      <AlertDialog open={!!pending} onOpenChange={open => !open && setPending(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction disabled={isSubmitting} className={cn(pending && ACTION_META[pending.action].destructive && "bg-destructive hover:bg-destructive/90 text-destructive-foreground")} onClick={e => { e.preventDefault(); handleConfirm(); }}>
