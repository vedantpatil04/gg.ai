  ...
  return (
    <div className="px-4 md:px-6 py-6 space-y-5">
      <SectionTitle
        ...
        action={
          <Button variant="outline" size="sm" onClick={() => qc.invalidateQueries({ queryKey: ["complaint-management"] })}>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <Input value={search} onChange={e => { setSearch(e.target.value); }} placeholder="Search title, ID, submitter…" className="pl-8 h-9" />
        ...
        <div className="glass rounded-2xl p-4 grid grid-cols-1 sm:grid-cols-3 gap-4 animate-in slide-in-from-top-2 duration-200">
          {[
            ...
            <div key={label} className="space-y-1.5">
              ...
              <select value={value} onChange={e => onChange(e.target.value)} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary/30">
        ...
        {isLoading ? (
          ...
          <div className="space-y-1.5">
            {complaints.map(c => {
              ...
              return (
                <div key={c._id} className="flex items-center gap-4 p-3.5 rounded-xl border border-border/60 bg-card hover:bg-muted/30 cursor-pointer transition-colors" onClick={() => setSelected(c)}>
