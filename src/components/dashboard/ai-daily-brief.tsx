...
function toneForOutdoor(g: OutdoorGuidance) {
  return g === "Recommended" ? "success" : g === "Limit" ? "warning" : ("destructive" as const);
}
function toneForTrend(t: PollutionTrend) {
  return t === "Improving" ? "success" : t === "Worsening" ? "destructive" : ("muted" as const);
}
function toneForWeather(w: WeatherImpact) {
  return w === "Favorable" ? "success" : ("muted" as const);
}
function toneForRisk(r: HealthRiskLabel) {
  return r === "Low" ? "success" : r === "Moderate" ? "warning" : ("destructive" as const);
}
  ...
  return (
    ...
    >
      ...
      <p className="relative text-sm text-muted-foreground leading-relaxed">
        ...
        {stillTyping && (
          <span
            className="inline-block w-0.5 h-3.5 bg-primary ml-0.5 animate-pulse align-text-bottom"
            aria-hidden
          />
        )}
      </p>
...
export function AIDailyBriefCard({
  ...
  userName?: string;
  cityName: string;
  leadInsight?: DailyBriefInsight;
  supportingInsights: DailyBriefInsight[];
  aqi: number;
  aqiLabel: string;
  outdoorGuidance: OutdoorGuidance;
  pollutionTrend: PollutionTrend;
  weatherImpact: WeatherImpact;
  healthRisk: HealthRiskLabel;
        ...
        <div className="flex items-start justify-between gap-3 mb-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            {!isLoading && !isError && !isEmpty && (
              <ShimmerBadge color={confColor}>{confidence.pct}% confidence</ShimmerBadge>
            )}
            <motion.button
            ...
            >
                ...
                <div className="space-y-2 pl-3 border-l-2 border-border/60">
                  {supportingInsights.map((ins) => (
                    <div key={ins.tag} className="text-xs text-muted-foreground leading-relaxed">
                      <span className="font-semibold text-foreground/80">{ins.tag}:</span>{" "}
                      {ins.body}
                    </div>
                  ))}
                </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                <InsightChip
                  label="Air Quality"
                  value={aqiLabel}
                  tone={toneForOutdoor(outdoorGuidance)}
                />
                <InsightChip
                  label="Outdoor Activity"
                  value={outdoorGuidance}
                  tone={toneForOutdoor(outdoorGuidance)}
                />
                <InsightChip
                  label="Pollution Trend"
                  value={pollutionTrend}
                  tone={toneForTrend(pollutionTrend)}
                />
                <InsightChip
                  label="Weather Impact"
                  value={weatherImpact}
                  tone={toneForWeather(weatherImpact)}
                />
                <InsightChip
                  label="Health Risk"
                  value={healthRisk}
                  tone={toneForRisk(healthRisk)}
                />
              </div>
              ...
              <div className="rounded-lg border border-border p-3">
                <div className="text-[11px] font-medium uppercase tracking-wider text-muted-foreground mb-2 flex items-center gap-1.5">
                  ...
                  {activityGuidance.mode === "recommended"
                    ? "Recommended today"
                    : activityGuidance.mode === "limit"
                      ? "Limit today"
                      : "Avoid today"}
                </div>
                <div className="flex flex-wrap gap-1.5">
