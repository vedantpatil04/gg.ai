      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {alerts.slice(0, 5).map((a) => (
            <div
              key={a.id ?? a._id}
              className="flex gap-3 rounded-xl border border-border p-3 hover:border-primary/40 transition-colors"
            >
              <div className="mt-0.5">
                {a.severity === "critical" && (
                  <span className="size-2 rounded-full bg-[var(--color-destructive)] block pulse-dot" />
                )}
                {a.severity === "warning" && (
                  <span className="size-2 rounded-full bg-[var(--color-warning)] block" />
                )}
                {a.severity === "info" && (
                  <span className="size-2 rounded-full bg-[var(--color-info)] block" />
                )}
              </div>
