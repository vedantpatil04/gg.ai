...
import {
  TrendingUp, TrendingDown, Minus, Zap, Droplets, Wind,
...
export function deriveTimelineEvents(
  ...
  for (let i = 1; i < series.length - 1; i++) { // exclude last (current)
  ...
  if (prev) {
    ...
    if (Math.abs(delta) >= 5) {
      nowDesc += delta < 0
        ? ` Down ${Math.abs(delta)} from yesterday.`
        : ` Up ${delta} from yesterday.`;
    }
  }
...
function eventIcon(kind: TimelineEvent["kind"], aqi: number) {
  if (kind === "now") {
    const color = aqi <= 50 ? "var(--color-success)" : aqi <= 100 ? "var(--color-warning)" : "var(--color-destructive)";
    ...
    <GlowCard glowVar="--glow-chart" lift={false} className="p-0">
      <div className="p-5 pb-0">
        <div className="flex items-start justify-between mb-4 gap-3">
          <div>
            <div className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">7-day history</div>
            <div className="text-base font-semibold tracking-tight mt-0.5">Environmental Timeline</div>
          </div>
        </div>
      </div>
              ...
              {events.map((event, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Content */}
                    <div
                      className="flex-1 rounded-xl p-3 transition-colors duration-150 group-hover:bg-muted/30"
                    >
