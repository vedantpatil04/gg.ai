...
// Routes through the GreenGuard AI Gateway (provider abstraction) instead of
// calling the Gemini SDK directly. Behavior — including every fallback
// message below — is unchanged from before Phase 1; only the internal path
// to Gemini changed (Gateway → Gemini Provider → Gemini SDK).
async function generate(prompt: string, system = SYSTEM_ENV_ANALYST): Promise<string> {
  try {
    const response = await aiGateway.generate({ prompt, systemInstruction: system });
    return response.text;
  } catch (err: unknown) {
    if (err instanceof AIProviderError) return _fallback(err.message);
    const msg = err instanceof Error ? err.message : String(err);
    logger.error(`AI Gateway error: ${msg}`);
